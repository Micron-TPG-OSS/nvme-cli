...
These only inspect what a command printed, so they apply equally to the
hardware-free suites under tests/cli/micron/ and the hardware suites under
tests/e2e/plugins/micron/.  The mixin expects the host class to provide
run_plugin_cmd(), run_plugin_cmd_check() and parse_json_output().
"""
