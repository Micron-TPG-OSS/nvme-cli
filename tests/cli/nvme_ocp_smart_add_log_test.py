...
Usage: python3 nvme_ocp_smart_add_log_test.py <nvme-binary> <mock-lib>
"""
import json
import os
import shutil
...
class TestOCPSmartAddLogOutputModes(OCPSmartAddLogTestBase):
    ...
    def test_output_format_version_is_ignored_for_text(self):
        """--output-format-version selects between JSON layouts only."""
        one = self.assertOk(self.run_smart('--output-format-version', '1'))
        two = self.assertOk(self.run_smart('--output-format-version', '2'))
    ...
    def test_invalid_output_format_is_rejected(self):
        result = self.run_smart('-o', 'bogus')
        self.assertNotEqual(result.returncode, 0,
                            '-o bogus must be refused')
        self.assertIn('invalid output format', result.stderr.lower())

    def test_unknown_output_format_version_is_rejected(self):
        """Only 1 and 2 exist. Anything else must be refused rather than
        silently rendered as some other layout."""
        for bad in ('0', '3', '99'):
            with self.subTest(version=bad):
                result = self.run_smart('-o', 'json',
                                        '--output-format-version', bad)
                self.assertNotEqual(
                    result.returncode, 0,
                    f'--output-format-version {bad} must be refused')
...
class TestOCPSmartAddLogErrors(OCPSmartAddLogTestBase):
    ...
    def test_nonexistent_device_fails(self):
        result = self.run_smart(device='/dev/nvme99n99')
        self.assertNotEqual(result.returncode, 0,
