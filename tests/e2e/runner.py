...
class E2ETestResult(unittest.TestResult):
    ...
    _OK = frozenset({'pass', 'expected_failure', 'skip'})
    ...
    def _test_method_of(test: unittest.TestCase) -> unittest.TestCase:
        ...
        parent test case in .test_case. Everything here counts and names
        test methods, so a wrapper has to resolve to its parent.
        """
    ...
    def _write_result(self, test: unittest.TestCase, status: str,
        ...
        self._stdout_stream.write('{} {} - {}{}\n'.format(
            'ok' if status in self._OK else 'not ok',
            self._test_count, self._description(test), directive))
        self._stdout_stream.flush()

    def _note(self, test: unittest.TestCase, status: str,
              directive: str = '', message: str | None = None) -> None:
        """Fold one reported outcome into the running test's buffered result.
        ...
        for an error out of tearDown. The plan written up front counts test
        methods, so all of those must collapse into one TAP line: the most
        severe outcome names it and every message is kept.
        """
        test = self._test_method_of(test)
    ...
    def stopTest(self, test: unittest.TestCase) -> None:
        ...
        Reporting from here rather than from the individual add*() hooks is
        what lets the worst outcome win, and is the only point at which the
        test's full duration is known -- unittest reports a failing subtest
    ...
    def addSubTest(self, test: unittest.TestCase, subtest: unittest.TestCase,
        ...
        self._note(test, status, message=tb)
    ...
    def addError(self, test: unittest.TestCase, err: object) -> None:
        ...
        self._note(test, 'error', message=tb)
        self._output_traceback(tb)

    def addFailure(self, test: unittest.TestCase, err: object) -> None:
        super().addFailure(test, err)
        tb = self._format_traceback(err)
        self._note(test, 'fail', message=tb)
        self._output_traceback(tb)

    def addSkip(self, test: unittest.TestCase, reason: str) -> None:
        super().addSkip(test, reason)
        self._note(test, 'skip', f' # SKIP {reason}', reason)

    def addExpectedFailure(self, test: unittest.TestCase, err: object) -> None:
        super().addExpectedFailure(test, err)
        self._note(test, 'expected_failure', ' # TODO expected failure',
                   self._format_traceback(err))

    def addUnexpectedSuccess(self, test: unittest.TestCase) -> None:
        super().addUnexpectedSuccess(test)
        self._note(test, 'unexpected_success', ' # TODO unexpected success')
