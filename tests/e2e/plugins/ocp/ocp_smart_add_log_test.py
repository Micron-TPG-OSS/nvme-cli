...
What only a real controller can show is whether the plugin decodes the
bytes that drive actually returned. So the tests here read the same log
page a second way, with a generic `nvme get-log`, and decode it against
the field table in ocp_c0_layout.py -- transcribed from the OCP
specification rather than from nvme-cli's own struct. A disagreement is
a decode bug: a wrong offset, width, or endianness.

Field values, version gating and the option surface are pinned down
exhaustively, at every log page version, by the hardware-free suite in
tests/cli/nvme_ocp_smart_add_log_test.py. This module deliberately does
not duplicate that; it covers what needs a drive.
