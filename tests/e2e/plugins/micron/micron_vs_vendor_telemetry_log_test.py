...
#
"""Tests for the micron vs-vendor-telemetry-log command.

vs-vendor-telemetry-log reports the drive's vendor telemetry log.  The
command is gated on the drive model derived from the PCI device ID and prints
"Unsupported drive model for vs-vendor-telemetry-log command" on any other
drive, so the gating contract is tested alongside the output and the output
assertions skip on a drive that cannot reach them.
...
_COMMAND = "vs-vendor-telemetry-log"
...
class TestMicronVsVendorTelemetryLog(TestMicron):
    """Test suite for the micron vs-vendor-telemetry-log command."""
    ...
    def test_json_field_object(self):
        """JSON output reports the telemetry log as one top-level key."""
