...
#ifndef ENOKEY
/*
 * If platform doesn't support ENOKEY, return ENOSYS instead to indicate
 * that the functionality is not supported.
 */
#define ENOKEY ENOSYS
#endif
		...
		const char *secret, size_t key_len, unsigned char **raw_secret)
{
	...
	if (key_len != 32 && key_len != 48 && key_len != 64) {
		libnvme_msg(ctx, LIBNVME_LOG_ERR, "Invalid key length %zu",
			   key_len);
		return -EINVAL;
	}
