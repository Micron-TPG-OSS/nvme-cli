...
\t# Complete an option's value from a known set, in both --opt=val and
\t# --opt val forms. Called once per command with alternating NAMES VALUES
\t# arguments: NAMES is a space-separated spelling list ('--output-format -o'),
\t# VALUES its space-separated value list. Completes the first option whose
\t# spelling matches the word being entered.
\t_nvme_opt_vals() {
\t\tlocal -a names
\t\tlocal name
\t\twhile (( $# >= 2 )); do
...
def emit_zsh_command_clause(out, tab, path, cmd):
    ...
    if not opts:
        ...
        out.write(f"{tab}_nvme_has_device || compadd -- /dev/nvme*(N)\n")
        out.write(f"{tab}\t;;\n")
        return
    ...
    out.write(f"{tab}\t--help':show help'\n")
    out.write(f"{tab}\t-h':alias for --help'\n")
    out.write(f"{tab})\n")
    out.write(f"{tab}_arguments '*:: :->subcmds'\n")
    plabel = f"nvme {path + ' ' if path else ''}{name} options"
    out.write(f'{tab}_nvme_describe "{plabel}" {var}\n')
    out.write(f"{tab}_nvme_has_device || compadd -- /dev/nvme*(N)\n")
    out.write(f"{tab}\t;;\n")
