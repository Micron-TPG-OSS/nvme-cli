...
# compadd -- /dev/nvme*(N)  -> record any device candidates (the (N) glob
# yields nothing on a host with no NVMe devices, so record the literal intent).
compadd() {
    local a
    for a in "$@"; do
        [[ $a == /dev/nvme* ]] && captured+=("/dev/nvme*")
    done
}
...
# help and version have no command clause, so they fall through to _files and
# never offer a device.
expect_no_device \
