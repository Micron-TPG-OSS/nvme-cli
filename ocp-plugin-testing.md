# OCP Plugin Test Coverage — Report and Next Steps

The OCP half of [plugin-test-coverage.md](plugin-test-coverage.md), extracted so
it can be worked and tracked on its own. That document also covers the micron
plugin and carries two micron follow-ups not repeated here.

## Context

The micron plugin's tests are built out in two layers: hardware-free mock
suites under [tests/cli/micron/](tests/cli/micron/) driven by
`libmock_nvme.so` + `MockIPCServer`, and hardware e2e suites under
[tests/e2e/plugins/micron/](tests/e2e/plugins/micron/). That gives OCP a
working pattern to copy.

OCP has nothing comparable. The plugin registers **30 commands**
([ocp-nvme.c:3385](plugins/ocp/ocp-nvme.c#L3385)) and exactly **one** of them
has a test. This document inventories the gap, then lays out a build order for
closing it — telemetry first, then by impact.

---

# Part 1: Coverage Report

## 30 commands — 29 untested
...
These are the concrete reasons the gap matters — each is the kind of thing the
micron suites' existing assertion patterns catch on the first run.
...
**Finding 3 — hardware alone can't reach most of this.** The drive available
for e2e reports C0 log page version 4, so even the existing `smart_add_log`
version-6 tests skip. Telemetry data area 4 additionally requires `lpa` bit 6
and an ETDAS round-trip ([ocp-nvme.c:1577](plugins/ocp/ocp-nvme.c#L1577)).
Hardware alone cannot cover the OCP matrix; the mock layer is not optional.
...
Key enabler for telemetry: `internal-log` accepts `-l` (telemetry binary) and
`-s` (C9 string binary) and skips the drive read when both are given
...
## Phase 1 — Telemetry (top priority)
...
## Phase 2 — Highest-impact remaining work
...
3. **`persistent-event-log` and `hardware-component-log`** — the two remaining
   log pages with real decode logic, each deserving its own file
   ([ocp-hardware-component-log.c](plugins/ocp/ocp-hardware-component-log.c),
   [ocp-nvme.c:3009](plugins/ocp/ocp-nvme.c#L3009)).
4. **Mutating commands, mock-only** — the nine setters/clears. Keep them off
   hardware; assert the issued FID, value, save and UUID bits against
   `server.commands`.
...
meson test -C .build --suite ocp-cli -v
```
...
```bash
meson setup .build -De2e-tests=true -Dplugin-tests=ocp \
...
Expected outcome per phase: Phase 1 lands 4 commands with real branch coverage
and fixes Findings 1 (C9) and 2. Phase 2 items 1–2 take OCP from 1/30 to
roughly 26/30 covered across two parametrised files, and close Finding 1
entirely.
...
exposes it. Keeping the work test-only instead means writing those assertions
against the *current* behaviour and marking them `expectedFailure` — decide
which before Phase 1 starts.
